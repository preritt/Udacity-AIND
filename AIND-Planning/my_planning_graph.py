...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        action_level = set()
        prior_state_nodes = self.s_levels[level]

        for possible_action in self.all_actions:
            pg_node_action = PgNode_a(possible_action) # generate an action node
            pre_conditions_for_this_action = pg_node_action.prenodes # check required precondition for this action
            if pre_conditions_for_this_action.issubset(prior_state_nodes): # => the preconditions for this action are satisfied              
                for nodes in prior_state_nodes:
                    if nodes in pre_conditions_for_this_action:
                        nodes.children.add(possible_action)
                        pg_node_action.parents.add(nodes)
                action_level.add(pg_node_action)
    ...
    def add_literal_level(self, level):
        ...
        literal_level = set() # Initialise the next literal level to be empty set
        all_actions_occuring = self.a_levels[level-1]
        for each_action in all_actions_occuring: # each action if of type PgNode_a
            effect_from_this_action = each_action.effnodes # this is a set of all the effects from this action
            for effect in effect_from_this_action:
                if effect in literal_level: # check if this effect has already been added to the s level
                    for element in literal_level:
                        if element == effect:
                            element.parents.add(effect)
                            each_action.children.add(element)
                else:
                    #
                    # this element has not yet been added to the literal level
                    # add this element to the literal level
                    effect.parents.add(each_action)
                    literal_level.add(effect)
                    each_action.children.add(effect)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        node_a1_actions = node_a1.action
        node_a1_actions_positive = node_a1_actions.effect_add
        node_a1_actions_negative = node_a1_actions.effect_rem

        node_a2_actions = node_a2.action
        node_a2_actions_positive = node_a2_actions.effect_add
        node_a2_actions_negative = node_a2_actions.effect_rem

        # inconsistent_effects = False
        for node1_pos_action in node_a1_actions_positive:
            if node1_pos_action in node_a2_actions_negative:
                return True

        for node2_pos_action in node_a2_actions_positive:
            if node2_pos_action in node_a1_actions_negative:
                return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        node_a1_actions_positive = node_a1_actions.effect_add
        node_a1_actions_negative = node_a1_actions.effect_rem 
        node_a1_actions_precond_pos =  node_a1_actions.precond_pos
        node_a1_actions_precond_neg= node_a1_actions.precond_neg
        node_a1_all_effects = list(set(node_a1_actions_positive).union(set(node_a1_actions_negative)))

        node_a2_actions = node_a2.action
        node_a2_actions_positive = node_a2_actions.effect_add
        node_a2_actions_negative = node_a2_actions.effect_rem
        node_a2_actions_precond_pos =  node_a2_actions.precond_pos
        node_a2_actions_precond_neg = node_a2_actions.precond_neg
        node_a2_all_effects = list(set(node_a2_actions_positive).union(set(node_a2_actions_negative)))


        for node_1_effects in node_a1_actions_negative:
            if node_1_effects in node_a2_actions_precond_pos:
                return True

        for node_2_effects in node_a2_actions_negative:
            if node_2_effects in node_a1_actions_precond_pos:
                return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        parent_node_1 = node_a1.parents
        parent_node_2 = node_a2.parents
        for parent_1 in parent_node_1:
            for parent_2 in parent_node_2:
                # if self.negation_mutex(parent_1,parent_2) or self.inconsistent_support_mutex(parent_1,parent_2):
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        if node_s1.symbol  == node_s2.symbol:
            if (node_s1.is_pos and not node_s2.is_pos) or (node_s2.is_pos and not node_s1.is_pos):
                return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        parent_node_1 = node_s1.parents # all actions that can achieve node_s1
        parent_node_2 = node_s2.parents # all actions that can achieve node_s1

        for parent_1 in parent_node_1:
            for parent_2 in parent_node_2:
                if (not(parent_1.is_mutex(parent_2)) or not(parent_2.is_mutex(parent_1))
                                or (node_s1 in parent_2.effnodes) or (node_s2 in parent_1.effnodes)):
                    inconsistency = False
    ...
    def h_levelsum(self) -> int:
        ...
        for goal_literal in self.problem.goal:
            found = False
            for (level, pg_nodes) in enumerate(self.s_levels):
                for pg_node in pg_nodes:
                    if pg_node.is_pos and pg_node.symbol == goal_literal:
                        level_sum += level
                        found = True
                        break
                if found:
